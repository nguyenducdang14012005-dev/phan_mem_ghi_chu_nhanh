...
export const getAllLabels = async (req, res) => {
  try {
    // Lấy đúng user_id từ Token đã giải mã
    const user_id = req.user.user_id ?? req.user.id;

    const result = await sql.query`
            SELECT * FROM Labels 
            WHERE user_id = ${user_id}`; // Chỉ lấy nhãn của ĐÚNG người đó
...
export const attachLabelToNote = async (req, res) => {
  try {
    ...
    const user_id = req.user.user_id ?? req.user.id; // Lấy từ token

    // BẮT BUỘC: Kiểm tra quyền sở hữu
    const checkOwnership = await sql.query`
    ...
    if (checkOwnership.recordset.length === 0) {
      return res
        .status(403)
        .json({ message: "Bạn không có quyền sử dụng nhãn này" });
    }
